...
impl ExpressionTaker for LoopTaker {
	...
	fn next_expression<'a>(&self, stream: &mut TokenStream, _args: Self::Args) -> ParseResult<Expression> {
		if let TokenTree::Token(Token { kind: TokenKind::Keyword(Keyword::Loop), .. }) = stream.take_tokentree()? { } else {
			return Ok(None)
		}
		...
		Ok(Some(Expression::Loop(Box::new(Expression::Block(Box::new(body))))))
	}
}
