...
pub fn take_typedef(in_tokens: &[TokenTree]) -> ParseResult<Type> {
	...
	let last_location = match tokens.get(0) {
		Some(TokenTree::Token(Token { kind: TokenKind::Keyword(Keyword::Type), location })) => {
			tokens = &tokens[1..];
			*location
		},
		_ => return Ok(None),
	...
	let (name, mut last_location) = match tokens.get(0) {
		Some(TokenTree::Token(Token { kind: TokenKind::Name(ref name), location })) => {
			...
			tokens = &tokens[1..];
			(TypeName::from_ident(Identifier::try_from_str(name)), *location)
		},
		...
		}.into()), // Type was not given a name
	};

	let mut fields = Vec::new();

	match tokens.get(0) {
		Some(TokenTree::Block(BlockType::Brace, ref type_tokens, start_location, end_location)) => {
			for field_tokens in separated::parse_separated(type_tokens, |token| token.is_comma())? {
				let name = match field_tokens.get(0) {
					...
					t => return Err(ParseError {
						kind: ParseErrorKind::Expected(vec![Expected::Identifier]),
						location: t.map(|t| t.get_location()).unwrap_or(last_location),
					}.into()), // Got a field that wasn't a name
				};
				...
				match field_tokens.get(1) {
					...
					t => return Err(ParseError {
						...
						location: t.map(|t| t.get_location()).unwrap_or(last_location),
					}.into()), // Field name didn't have a colon after it
				}
		...
		t => return Err(ParseError {
			...
			location: t.map(|t| t.get_location()).unwrap_or(last_location)
		}.into()), // needed a a block with fields
	};
