		...
		match in_tokens.get(0) {
			Some(TokenTree::Token(Token::Keyword(Keyword::Loop))) => {
				...
				if let Some(TokenTree::Brace(_)) = in_tokens.get(1) {
					if let Some((block, extra_leftovers)) = next_expression(&in_tokens[1..], Box::new(|token| !token.is_brace_expr()))? {
						Ok(Some((Expression::Loop(Box::new(block)), extra_leftovers)))
					} else {
						Err(ParseError::Other.into()) // Failed to parse the block after the loop
					}
				} else {
					if let Some((expr, extra_leftovers)) = next_expression(&in_tokens[1..], Box::new(|token| token.is_semicolon()))? {
						Ok(Some((Expression::Loop(Box::new(Expression::Block(Box::new(Block {
							block: vec![expr],
							output: None,
						})))), extra_leftovers)))
					} else {
						Err(ParseError::Other.into()) // Failed to parse the expression after the loop
					}
				}
			},
			_ => Ok(None)
