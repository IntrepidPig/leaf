...
pub fn next_functiondef(stream: &mut TokenStream) -> ParseResult<FunctionDefinition> {
	...
	let name = if let TokenTree::Token(Token {
	...
	let args: Vec<(Identifier, PathItem<TypeName>)> =
		if let TokenTree::Block(Bracket::Paren, ref mut args_token_stream, _, _) = stream.take_tokentree()? {
			...
			for mut args_token_stream in args_token_streams {
				let ident = if let TokenTree::Token(Token {
					kind: TokenKind::Name(ref name),
					..
				}) = args_token_stream.take_tokentree()?
				{
					Identifier::try_from_str(name)
				} else {
