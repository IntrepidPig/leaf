...
pub fn take_functiondef(in_tokens: &[TokenTree]) -> ParseResult<Function> {
	...
	let last_location = match tokens.get(0) {
		Some(TokenTree::Token(Token { kind: TokenKind::Keyword(Keyword::Function), location })) => {
			tokens = &tokens[1..];
			*location
		},
		_ => return Ok(None),
	...
	let (args, last_location) = match tokens.get(0) {
		Some(TokenTree::Block(BlockType::Paren, ref args_tokens, start_location, end_location)) => {
			...
			if !args_tokens.is_empty() {
				(parse_args(args_tokens, *start_location)?, args_tokens[0].get_location())
			} else {
		...
		t => return Err(ParseError {
			...
			location: t.map(|t| t.get_location()).unwrap_or(last_location),
		}.into()), // needed parens with arguments inside
	};
	...
	let (return_type, last_location) = match tokens.get(0) {
		Some(TokenTree::Token(Token { kind: TokenKind::Symbol(TokenSymbol::Colon), location })) => {
			...
			tokens = leftovers;
			(Some(typename), leftovers.get(0).map(|t| t.get_location()).unwrap_or(*location))
		},
...
pub fn take_externfn(in_tokens: &[TokenTree]) -> ParseResult<ExternFunction> {
	...
	match tokens.get(0) {
		Some(TokenTree::Token(Token { kind: TokenKind::Keyword(Keyword::Extern), .. })) => {
			tokens = &tokens[1..];
		},
		_ => return Ok(None),
	}
...
fn parse_args(in_tokens: &[TokenTree], start_location: Location) -> Result<Vec<(Identifier, PathItem<TypeName>)>, Error<ParseError>> {
	...
	for arg_tokens in separated::parse_separated(in_tokens, |token| token.is_comma())? {
		let name = match arg_tokens.get(0) {
			...
			t => return Err(ParseError {
				kind: ParseErrorKind::Expected(vec![Expected::Identifier]),
				location: t.map(|t| t.get_location()).unwrap_or(last_location),
			}.into()), // Got an argument that wasn't a name
		};
		...
		match arg_tokens.get(1) {
			...
			t => return Err(ParseError {
				...
				location: t.map(|t| t.get_location()).unwrap_or(last_location)
			}.into()), // Argument name needed colon after it
		}

		let (typename, leftovers) = if let Some(res) = next_type(&arg_tokens[2..])? { // TODO remove panic
			res
		} else {
			return Err(ParseError {
				kind: ParseErrorKind::Expected(vec![Expected::Typename]),
