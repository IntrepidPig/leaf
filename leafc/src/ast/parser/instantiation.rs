...
impl ExpressionTaker for InstantiationTaker {
	...
	fn take_expression<'a>(&self, in_tokens: &'a [TokenTree], _args: Self::Args) -> ParseResult<'a, Expression> {
		...
		match tokens.get(0) {
			Some(TokenTree::Block(BlockType::Brace, ref tokens, start_location, end_location)) => {
				...
				for field_tokens in separated::parse_separated(tokens, |token| token.is_comma())? {
					let name = match field_tokens.get(0) {
						Some(TokenTree::Token(Token { kind: TokenKind::Name(ref ident), location })) => {
					...
					let (expr, _leftovers) =
						if let Some(expr) = next_expression(&field_tokens[2..], Box::new(|_| false))? { // TODO remove panic
							expr
						} else {
