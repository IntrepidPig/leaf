use ast::tokenizer::{Token, TokenKind, Symbol, Keyword};
use ast::lexer::{Span, Location, Bracket, BracketState};
use ast::parser::Expected;
...
impl<'a> TokenStream<'a> {
	...
	pub fn opt_next_tokentree(&mut self) -> Result<Option<TokenTree<'a>>, Error<ParseError>> {
		...
		let tokentree = match token {
			Token { kind: TokenKind::Bracket(bracket, BracketState::Open), .. } => {
				...
				while let Some(test_token) = self.next_token() {
					match test_token {
						Token { kind: TokenKind::Bracket(test_bracket, test_bracket_state), span } => {
