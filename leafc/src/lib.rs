...
pub fn leafc_str<P: AsRef<Path>>(input: &str, core_path: P, includes: &[P]) -> Result<Vec<Instruction>, failure::GenericError> {
	let mut includes: Vec<(String, &Path)> = {
		includes.iter().map(|include_path| {
...
pub fn leafc<P: AsRef<Path>>(path: P, core_path: P, includes: &[P]) -> Result<Vec<Instruction>, failure::GenericError> {
	let mut includes: Vec<(String, &Path)> = {
		includes.iter().map(|include_path| {
			let include_path = include_path.as_ref();
			let include_file_name = include_path.file_name().unwrap().to_str().unwrap();
